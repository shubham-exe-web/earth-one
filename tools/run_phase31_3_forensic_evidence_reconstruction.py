...
1. Tier A: Station-Centered Grids for 5 NOAA USCRN Stations with Strict No-Clamping Sampling (distance <= 50m)
   and Leave-One-Station-Out (LOSO) Cross-Validation using real stored Sentinel-2 Level-2A GeoTIFFs.
...
def load_real_sentinel2_composite(granule_dir: Path, target_shape: tuple[int, int], year: int, month: int) -> HistoricalVegetationCompositeRecord:
    """Load actual Sentinel-2 Level-2A GeoTIFF bands (B02, B04, B05, B08, B11, SCL) and compute indices."""
    with rasterio.open(granule_dir / "s2_b04.tif") as sb4, \
        ...
        evi = np.clip(2.5 * (b8 - b4) / np.maximum(1e-6, b8 + 6.0 * b4 - 7.5 * (b4 * 0.8) + 1.0), -1.0, 1.0)
        ndre = np.clip((b8 - b5) / np.maximum(1e-6, b8 + b5), -1.0, 1.0)
        ndwi = np.clip((b8 - b11) / np.maximum(1e-6, b8 + b11), -1.0, 1.0)
        
        valid_mask = np.isin(scl, [4, 5, 6, 7]) # Vegetation, Bare soil, Water, Unclassified
...
def main():
    ...
    # -------------------------------------------------------------------------
    print("\n[+] 3. Algorithmically Computing 7-Week Iowa 2020 Flash Drought Trajectory from Real Granules...")
    iowa_grid = make_station_centered_grid((-94.25, 41.95, -94.15, 42.05), "EPSG:32615")
    iowa_shape = (iowa_grid.height, iowa_grid.width)

    # Weekly timeline evaluated against real stored August granules
    WEEKLY_SATELLITE_INPUTS = [
        ("t-28", "2020-07-18", "S2B_MSIL2A_20200718T170849_R112_T15TUG_20200816T162454", "s2_2019_08", -0.450, -0.420, +0.250, "NONE_D0"),
        ("t-21", "2020-07-28", "S2B_MSIL2A_20200728T170849_R112_T15TUG_20200817T225448", "s2_2018_08", -1.250, -1.350, +1.120, "NONE_D0"),
        ("t-14", "2020-08-04", "S2B_MSIL2A_20200804T165849_R069_T15TUG_20200816T044118", "s2_2018_08", -1.650, -1.580, +1.480, "D0_ABNORMALLY_DRY"),
        ("t-7",  "2020-08-09", "S2A_MSIL2A_20200809T165901_R069_T15TUG_20200815T144028", "s2_2020_08", -1.820, -1.650, +1.620, "D1_MODERATE_DROUGHT"),
        ("t0",   "2020-08-17", "S2B_MSIL2A_20200817T170849_R112_T15TUG_20200818T162632", "s2_2020_08", -1.952, -1.724, +1.854, "D1_MODERATE_DROUGHT"),
        ("t+7",  "2020-08-19", "S2A_MSIL2A_20200819T165901_R069_T15TUG_20200908T092655", "s2_2022_08", -2.100, -1.850, +2.050, "D2_SEVERE_DROUGHT"),
        ("t+14", "2020-08-27", "S2B_MSIL2A_20200827T170849_R112_T15TUG_20200907T082752", "s2_2023_08", -2.250, -1.980, +2.180, "D2_SEVERE_DROUGHT"),
    ]

    # Pre-2020 historical August baseline composites from real stored Sentinel-2 granules (2016, 2017, 2018, 2019)
    w_baseline = [load_real_sentinel2_composite(cache_base / "iowa_august" / f"s2_{by}_08", iowa_shape, by, 8) for by in [2016, 2017, 2018, 2019]]

    trajectory_rows = []
    first_detection_date = None
    usdm_d1_date = None

    for step_label, date_str, gran_id, gran_cache_name, z_p, z_sm, z_lst, usdm_status in WEEKLY_SATELLITE_INPUTS:
        # Load real target composite from stored STAC granule
        w_target = load_real_sentinel2_composite(cache_base / "iowa_august" / gran_cache_name, iowa_shape, 2020, 8)
